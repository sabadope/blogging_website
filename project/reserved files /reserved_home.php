...
<head>
   ...
   <style>
        ...
        /* Show team members based on filter */
        .team-row .img-box.show {
            display: block;
        }

        .divider {
            color: #ffffff;
            font-size: 20px;  /* Adjust font size to match the text */
        ...
        .chat-body::-webkit-scrollbar {
            ...
            border-radius: 20px; /* Add rounded corners to the scrollbar */
            z-index: 1999;
        }
    ...
    <div class="chatbot-popup">
        ...
        <div class="chat-body">
            <div class="message bot-message thinking">
                ...
                <div class="message-text">Hi there! 👋 Having trouble with your reading?<br><br> Don't worry, I'm here to listen.</div>
            </div>
        </div>        
...
<section class="home" id="home">
   ...
   <div class="content" style="width: 90%;">
      <h1 style="color: #fff; font-size: 4rem;">Practicality at Its Finest: Simple and Effective Web-Based Book Archiving</h1>
      <p style="color: #c8c3bc;">A simple and effective tool for archiving web development and UI/UX design learning materials, this web-based system ensures seamless organization. It provides users with quick access to their entire book collection, whenever needed.</p>
...
<div class="notifications">
    ...
    <?php if (!empty($message)): ?>
        <?php foreach ($message as $msg): ?>
            <?php if ($msg['type'] === 'info'): ?>
                <div class="toast info">
                    <i class="bx bx-info-circle"></i>
                    <div>
                        <div class="title"><?= ucfirst($msg['type']) ?></div>
...
<script>
   ...
   blurBackground.style.display = 'none';
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // Get all filter text links
        const filterButtons = document.querySelectorAll('.filter-btn-team');
        
        // Get all team member containers
        const teamMembers = document.querySelectorAll('.team-row .img-box');
        
        // Function to filter team members based on the selected category
        function filterTeamMembers(filter) {
            teamMembers.forEach(member => {
                if (member.classList.contains(filter) || filter === 'all') {
        ...
        // Set the default filter to 'programmer' when the page loads
        filterTeamMembers('programmer');

        // Loop through each filter link and add an event listener
        filterButtons.forEach(button => {
            button.addEventListener('click', () => {
                // Get the filter type from the button's data-filter attribute
                const filter = button.getAttribute('data-filter');
                
                // Call the function to filter the team members
...
<script>
   ...
   // API setup
   const API_KEY = "AIzaSyDM82rZMbnn3obu1uUFTF187hjqbQAunCM"; // Insert your own API key here
   const API_URL = `https://generativelanguage.googleapis.com/v1beta2/models/gemini-1.5:generateMessage?key=${API_KEY}`;
   ...
   const generateBotResponse = async (incomingMessageDiv) => {
     ...
     try {
       ...
       const botMessage = data.candidates[0].content.trim();
   ...
   messageInput.addEventListener("keydown", (e) => {
     ...
     if (e.key === "Enter" && userMessage) {
       e.preventDefault(); // Prevent new line if input is a textarea
